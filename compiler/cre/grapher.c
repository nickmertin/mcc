            ...
            case CRE_CHAR:
                ...
                for (size_t j = 0; j < 256; ++j) {
                    if (getFlag(tokens[i].filter, j)) {
                        ...
                        block_builder_add_statement(&builder, (struct cg_statement) {.type = CG_IFELSE, .data.ifelse = {.cond_var = comparison, .if_true = generate_jump_block(&builder, strdup(label)), .if_false = { .statement_count = 0, .variable_count = 0 }}});
