...
static struct cg_block generate_jump_block(char *label) {
    ...
    *statement = (struct cg_statement) {.type = CG_JUMP, .data.jump.label = label};
...
static struct cg_block generate_return_block(bool result) {
    ...
    statements[0] = (struct cg_statement) {.type = CG_ASSIGN, .data.assign = {.var = 0, .expr = {.type = CG_VALUE, .data.value.value =  result ? 1 : 0}}};
    statements[1] = (struct cg_statement) {.type = CG_ENDFUNC, .data.endfunc = {.var = 0}};
            ...
            case CRE_CHAR:
                for (size_t j = 0; j < 256; ++j) {
                    ...
                    if (getFlag(tokens[i].filter, j)) {
                        stmt = (struct cg_statement) {.type = CG_ASSIGN, .data.assign = {.var = 2, .expr = {.type = CG_BINARY, .data.binary = {.type = CG_ADD, .left_var = 0, .right_var = 1}}}};
                        linked_list_insert(statements, 0, &stmt, sizeof(struct cg_statement));
                        stmt.data.assign = (struct cg_statement_assign) {.var = 3, .expr = {.type = CG_UNARY, .data.unary = {.var = 2, .type = CG_DEREF}}};
                        linked_list_insert(statements, 0, &stmt, sizeof(struct cg_statement));
                        stmt.data.assign = (struct cg_statement_assign) {.var = 4, .expr = {.type = CG_VALUE, .data.value.value = 0}};
                        linked_list_insert(statements, 0, &stmt, sizeof(struct cg_statement));
                        stmt.data.assign = (struct cg_statement_assign) {.var = 3, .expr = {.type = CG_BINARY, .data.binary = {.type = CG_EQ, .left_var = 3, .right_var = 4}}};
                        linked_list_insert(statements, 0, &stmt, sizeof(struct cg_statement));
                        stmt = (struct cg_statement) {.type = CG_IFELSE, .data.ifelse = {.cond_var = 3, .if_true = generate_jump_block(strdup(label)), .if_false = generate_return_block(false)}};
                        linked_list_insert(statements, 0, &stmt, sizeof(struct cg_statement));
                    }
                    stmt = (struct cg_statement) {.type = CG_LABEL, .data.label.label = strdup(label)};
                    linked_list_insert(statements, 0, &stmt, sizeof(struct cg_statement));
                }
                break;
        }
